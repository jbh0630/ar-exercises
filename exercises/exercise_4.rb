...
@mens_stores.each { |mens_store| puts "Men Store Name: #{mens_store.name} Revenue: #{mens_store.annual_revenue}" }

@womens_stores = Store.where("womens_apparel = true")
                      .where("annual_revenue < 1000000")

@womens_stores.each { |womens_store| puts "Women Store Name: #{womens_store.name} Revenue: #{womens_store.annual_revenue}"}